...
| DBG-32 | 变异脚本报 `0/9 KILLED`——其实一条规则都没跑到 | 报“全活”之前先确认测试真的运行过；验证工具本身也要被验证 |
| DBG-33 | 一次性 `migrate` 容器起不来——entrypoint 入口类少写一层包名 | 编排里写死的类名也要有断言，别等容器启动才发现（`ComposeEntrypointTest`） |
| DBG-34 | `migrate` 连不上 mysql——健康检查在“还没监听 3306”时报了健康 | 健康检查要按应用真正走的协议与地址测（`-h 127.0.0.1 --protocol=TCP`） |
