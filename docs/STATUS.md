...
7. **P6 — 进行中**：
   ...
   - ✅ 部署编排（D-37）：`frontend` 服务（Nginx 托管产物 + 反代 `/api`→`backend:8080`、`/ws`→`backend:18080`）+ `socketUrl` 同源模式（`VITE_WS_SAME_ORIGIN`，构建期由 `frontend/Dockerfile` 打开）；`docker compose config` 与 `nginx -t` 通过，前端 **73** 单测与 `VITE_WS_SAME_ORIGIN=1 npm run build` 通过；Agent API 保持 `:8090` 独立不经反代。后续候选（本轮未做）：CI（GitHub Actions + MySQL service）、调度器/迁移开关、拆 `App.vue`。另按 D-38 收敛了竞拍 Agent 凭据的来源（只读 `AUCTION_AGENT_TOKEN` 或同名参数，不做交互输入，缺了就报错退 2），`tools/agent_sim.py --agent-only --auction-id <id>` 可只用评审自己的 Token 参与已有拍卖。
