...
| `DESIGN.md` | 架构边界、出价事务、结算、恢复 | ✅ 已同步 §1.6（V1~V6、Agent 边界、尾段博弈时间与成交主体、预告开拍与托管代理、239 证据与“尚未实现”）、§2.2~§2.4（分层与依赖规则）、§4（流水主体）、§8（验证重点）；P5 的 Agent 边界与 P6 的博弈时间/主体标识/托管代理已补入 |
...
7. **P6 — 进行中**：
   ...
   - ✅ 部署编排（D-37）：`frontend` 服务（Nginx 托管产物 + 反代 `/api`→`backend:8080`、`/ws`→`backend:18080`）+ `socketUrl` 同源模式（`VITE_WS_SAME_ORIGIN`，构建期由 `frontend/Dockerfile` 打开）；`docker compose config` 与 `nginx -t` 通过，前端 **73** 单测与 `VITE_WS_SAME_ORIGIN=1 npm run build` 通过；Agent API 保持 `:8090` 独立不经反代。CI 已落地并跑通（`.github/workflows/ci.yml`，5 个 job：后端 239、前端 73、`tools/*.py` 端到端 44+52、配置自检、镜像构建 + 整栈起来验 `:8080`/`:8088`；[run #2](https://github.com/Ayong-ui/bid-arena/actions/runs/34802520376) 全绿，见 §3）。**部署优化的第一个候选（D-39，迁移收敛）已完成**：`bootstrap/MigrateMain` 一次性迁移入口 + `MIGRATE_ON_START` 开关 + compose 的 `migrate` 服务（见 DECISIONS D-39 与本节下面那条）。仍待做：拆 `App.vue`（纯代码卫生，行为零变化）、演示录屏（G7）。（三个扫描器的开关已由 D-40 完成；真正的多实例自动分工仍不做，见 README 未完成边界。）另按 D-38 收敛了竞拍 Agent 凭据的来源（只读 `AUCTION_AGENT_TOKEN` 或同名参数，不做交互输入，缺了就报错退 2），`tools/agent_sim.py --agent-only --auction-id <id>` 可只用评审自己的 Token 参与已有拍卖。
