...
| 文件 | 权威范围 | 状态 |
|---|---|---|
| `README.md` | 快速启动完整路径、演示账号、**未完成边界** | ✅ 已按 D-42 重写：§2 快速启动（前置/环境变量/一条命令/登录）、§3 八步教程（每步“会看到什么”+ 卡点表）、§4 部署教材（拓扑与端口、`.env` 速查、五种部署形态、迁移与初始化、多实例、排错、停止/清库/复位）、§5 本地开发、§6 测试与持续集成（一辇命令与 242 构成、守卫与变异、CI 五个 job）、§7 工具脚本、§8 接口与 WS（折叠）、§9 项目结构、§10 文档地图、§11 未完成边界、§12 仓库约定；目录锚点与仓内链接有脚本校验 |
| `AI_USAGE.md` | AI 分工比例、本人设计决定、未采用方案、真实错误（原文必交） | ✅ 已填写：工具与模型、人机分工与口径、四项本人决定、六项未采用方案、四项真实错误（含两条未入 `DEBUG_LOG.md` 的补充）、七类尚不能独立解释的代码；文末附三项作者核对清单 |
| `DEBUG_LOG.md` | 开发期间真实问题的现象/日志/定位/修复/验证（原文必交） | ✅ 已写 DBG-1~DBG-34（文首索引已同步补齐至 DBG-34） |
| `AGENT_TOOL_SPEC.md` | 评审如何用 Token 让 Coding Agent 查询与出价（原文必交） | ✅ 已更新为 P5 已实现并验证：Token 形态/两条签发路径、操作步骤、提示词模板、失败边界、验收清单；开头新增“托管代理 vs Token”分岔说明（D-36） |
...
| `CONTRIBUTING.md` | 提交规范、完成定义（DoD）、GitHub 仓库管理、交付清单（含录屏分镜 §9.1.1与现场核验演练 §9.4） | ✅ 已创建 |
...
具体提交规范与 DoD 细则见 `CONTRIBUTING.md`（已创建；§9.1.1 是录屏分镜，§9.4 是 H 组现场核验演练稿）。
