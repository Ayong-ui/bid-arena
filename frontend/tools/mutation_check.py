...
MUTATIONS = [
    ...
    (
        "F11",
        "去重键丢掉 type（同一次提交的 AUCTION_EXTENDED 被误杀）",
        os.path.join("src", "realtime", "events.ts"),
        "  return `${event.auctionId}:${event.seq}:${event.type}`",
        "  return `${event.auctionId}:${event.seq}`",
        os.path.join("src", "realtime", "feed.test.ts"),
    ),
    (
        "F12",
        "不检测 seq 缺口（跳着应用，本地会缺一次状态变更）",
        os.path.join("src", "realtime", "feed.ts"),
    ...
    (
        "F13",
        "快照恢复时不丢弃旧事件（同一笔出价被叠加两次）",
        os.path.join("src", "realtime", "feed.ts"),
        "    const carryOver = this.buffer.filter((event) => event.seq > snapshot.seq)",
        "    const carryOver = this.buffer.filter((event) => event.seq >= 0)",
        os.path.join("src", "realtime", "feed.test.ts"),
    ),
    (
        "F14",
        "出价重试不复用幂等键（网络抖动会变成两次出价）",
        os.path.join("src", "store", "arena.ts"),
        ...
        os.path.join("src", "store", "arena.test.ts"),
    ),
    (
        "F15",
        "倒计时改用本机时钟（时钟不同步时把进行中的拍卖算成已结束）",
        os.path.join("src", "store", "arena.ts"),
    ...
    (
        "F16",
        "NOT_JOINED 不再自动加入后重试（用户点一次出价却什么都没发生）",
        os.path.join("src", "store", "arena.ts"),
