...
**背景**：本机 Windows 需要 MySQL 8；目标 VM（`192.168.x.x`，记为 `anolis-docker`）是**共享中间件开发机**，已有大量容器占用端口。原环境变量指向 `DOCKER_HOST=tcp://...:2376` + `D:\docker-remote` 的 TLS 证书，但该证书 SAN 只包含旧地址，**无法用于当前 IP**。
...
| `nmcli` / `ip addr` | `192.168.x.x/24`，`manual`，网关 `192.168.x.x`，默认路由 `proto static`（地址按 §8.3“不把内网拓扑写进仓库”匿名化） |
...
## D-42　README 以「五分钟能跑 + 八步教程 + 部署教材」为主轴重写，参考细节收进折叠区

**背景**：交付要求（原文 §“交付与验收”/README 一节）把 README 的权威范围定为**快速启动完整路径、演示账号、未完成边界**：评审要能照着 README 在 5 分钟内起栈，并按固定顺序（复制环境变量 → Compose 启动依赖 → 自动迁移与初始化 → 管理员开始演示拍卖 → 两个浏览器用户加入 → 20 人模拟脚本 → 建 Agent Token 让竞拍 Agent 参与 → 看成交/钱包/流水 → 跑自动化测试）走完整条链路。而改写前的 README 是“按功能模块平铺”的长文：代码块多、但**路径不显式**（读者要知道“先点哪、再看哪”得自己拼），长表格（接口清单、配置项、压测细节）把教程淹了；同时校对时查出 5 处与代码不一致（**不存在 `POST /auth/register`**、“拍卖列表/详情是公开接口”（实际除 `login` 与 `health` 外全要令牌）、吊销写成 `DELETE`（实际是 `POST .../{id}/revoke`）、登录响应字段写成 `data.token`（实际 `data.accessToken`）、示例 id 写成 `"agentUserId": 2` / `"auctionIds": ["1"]`（契约里都是字符串且真实 id 形如 `auc_demo_0001`））。

**候选方案**

| 维度 | 候选 | 说明 | 否决原因 |
|---|---|---|---|
| 主轴 | 保留“按模块平铺 + 补一个快速启动 | 改动最小 | 平铺结构里教程天然被打散，“下一步做什么”仍需读者自己接 |
| 主轴 | **以“五分钟启动 → 八步教程 → 部署教材”为主轴**（当前实现） | 先能跑通，再谈细节 | —— |
| 细节去哪 | 接口清单/WS 接入/压测细节照旧平铺 | 信息不丢 | 教程被长表淹没（原文关心的是路径，接口契约权威在 `docs/openapi.yaml`） |
| 细节去哪 | **收进 `<details>` 折叠区 + 指向权威文件**（当前实现） | 该在的还在，但不抢注意力 | 折叠内容不易被搜索命中；靠目录与关键词双入口缓解 |
| 文档量 | 只写“怎么跑”，配置/排错靠读者自己推测 | 最短 | “方便他人在本地部署”正是本轮优化目标，缺配置速查与排错表等于把成本转给部署者 |
| 数字口径 | 只写“测试全部通过” | 不用同步 | 无法验证；“做到什么”必须能被重跑，沿用 D-41 的口径 |
| 教程真实性 | 只写命令，不写“你会看到什么” | 写得快 | 卡住时无法自判“是我错了还是它就这样” |
| 教程真实性 | **每一步都给“会看到什么”+ 一张卡点表**（当前实现） | 自服务排错 | 写作成本高，且每个断言都得真的跑到 |
| 校对方式 | 靠人眼通读 | 无成本 | 平铺长文里的字段名/路由/默认值正是最容易出错的地方，且错了看不出来 |
| 校对方式 | **拿运行中的实例把教程命令原样跑一遍，逐个字段核对**（当前实现） | 失实当场暴露 | 要真实起后/前端与真库（本轮已具备） |

**最终选择**：重写为“5 分钟起步 → 八步教程（每步“会看到什么”+ 卡点表）→ 部署教材（拓扑/端口/配置速查/五种部署形态/多实例/排错/复位）→ 本地开发 → 测试与持续集成（口径与 `STATUS.md` 一致）→ 工具/接口折叠区 → 文档地图 → 未完成边界 → 仓库约定”；长表收进 `<details>`，接口契约指向 `docs/openapi.yaml`；同时保留其他文件已引用的章节名（`未完成边界`、`持续集成`）。目录由脚本按 GitHub 锚点规则生成并逐个校验可达（38 个标题、0 个失效锚点、13 个仓内相对链接全部可达）。

**代价**：① README 从 ~450 行涨到 ~690 行，但首屏 30 行内就是可复制的一整段启动命令，信息密度从“均匀地高”变成“按需展开”；② 每步的“会看到什么”都是会过期的断言，后续改响应字段就必须同步改 README（现在至少能靠 §6 的一键命令发现测试挂了）；③ 折叠区在页面内搜索时需要展开。

**验证结果**：✅ **已实测**：

| 证据 | 结果 |
|---|---|
| 起真库 + 从源码起后端（8080/8090/18080）与前端（5173），按 README 教程原样执行 | `join`→`OK`；`bid 110`→`accepted=true seq=3`；**同一 `requestId` 重放 → `IDEMPOTENCY_REPLAY` 且 `idempotent=true`**（不重复扣钱） |
| 冻结/释放不变量 | `bidder_b` 反超 130 后 `bidder_a` `frozenAmount=0`（已释放）、`bidder_b` `frozen=130`；同一人加价到 150 时 `frozen=150`（**只补差额**，不是 130+150） |
| 自助签发 Agent Token | `POST /me/agent-tokens` `OK`，`data.token` 长度 43（库里只存摘要） |
| `agent_sim.py --agent-only --auction-id auc_demo_0001 --bid` | `1/1 checks passed`，Agent 出价 160 成功（走 `:8090`） |
| 结算与隐私 | 到点结算 `reason=TIMEOUT`、`winnerType=AGENT`（赢家视角）/`winnerType` 缺失（他人视角）；赢家 `total=840 frozen=0`、输家 `total=1000 frozen=0`；流水出现 `FREEZE`→`RELEASE`→`SETTLE` 且 `actorType=AGENT` |
| 复位脚本 | `db/reset_demo_data.sql` 跑完回显三钱包 `1000/0` 与 `auc_demo_0001=DRAFT`（演示数据已复位） |
| 文档自检 | 38 个标题的目录锚点 0 失效；13 个仓内相对链接 0 缺失；未知路由/字段按运行实例修正 5 处 |
| 回归 | 本次只改文档（README/CONTRIBUTING/DECISIONS），未动 `src/`、未新增配置键，故不重跑 `mvn clean verify`（仍为 242/242）；提交后由 CI 复核 |
...
| README 继续按功能模块平铺、另加一节“快速开始” | 平铺结构里教程被打散，“下一步点哪”仍需读者自己拼；也压不住长表格对首屏的占用 | 重写成“教程在前、参考在后”（D-42） |
| 把接口清单/配置全表从 README 删掉、只留链接 | 评审要翻两个文件才能对着跑；部署者最常查的配置与端口应当就地可见 | 长表收进 `<details>`，契约细节指向 `docs/openapi.yaml`（D-42） |
| README 里的示例只写命令、不写预期输出 | 卡住时分不清“我错了”还是“它本来就长这样” | 每步都写“会看到什么”，并拿运行实例逐条核对（D-42） |
| 顺手把内网 IP 留在 `DECISIONS.md` 里（“反正是私有地址”） | 私有网段+网关能反推出内网拓扑；仓库是公开的 | 匿名成 `192.168.x.x`，并在 `CONTRIBUTING.md` §8.3 补成明文规则（D-42 同一轮校对发现） |
